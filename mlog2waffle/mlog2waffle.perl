...
sub send_event
{
   ...
   while ( my $line = $queue->dequeue ) {
      ...
      if (!$response->is_success) {
         ...
         my $timestamp = strftime('[%d/%b/%Y %H:%M:%S %z]',localtime); 
#         if (unlink $file_name) {
         if (move ($file_name, "/var/log/mlog2waffle/data/backup/" . basename($file_name))) {
            $logLine = "$timestamp Event $summary sent to console with status, file deleted\n";
            $logQueue->enqueue($logLine);
            # remove empty dirs, edit Vlasta
            rmdir(dirname($file_name));
            my $parent_dir = (split('/', $file_name))[-3];
            rmdir($MODSEC_DIRECTORY . $parent_dir);
         } else {
...
print "Replacing env params\n";

system('sed -i -e "s#CONSOLE_URI_PARAM#$CONSOLE_URI_PARAM#g" /etc/mlog2waffle.conf');
system('sed -i -e "s/CONSOLE_USERNAME_PARAM/$CONSOLE_USERNAME_PARAM/g" /etc/mlog2waffle.conf');
system('sed -i -e "s/CONSOLE_PASSWORD_PARAM/$CONSOLE_PASSWORD_PARAM/g" /etc/mlog2waffle.conf');
system('sed -i -e "s/DEBUG_PARAM/$DEBUG_PARAM/g" /etc/mlog2waffle.conf');
system('mkdir /var/log/mlog2waffle/data/backup');
